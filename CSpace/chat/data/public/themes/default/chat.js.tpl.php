...
//require_once dirname(__FILE__).'/../../src/commands/quit.class.php';
$json = new pfcJSON();
?>
...
var pfc_userchan              = <?php //echo $json->encode($c->channels);
                                         //_resetChannelIdentifier(md5(uniqid(rand(), true)));
                                        //$list = array(); foreach($u->channels as $item) {$list[] = $item["name"];} echo $json->encode($list);
//                                            $clientid    = $p["clientid"];
//                                            $param       = $p["param"];
//                                            $sender      = $p["sender"];
